...
async function fetchDailySalesData() {
    try {
      const response = await fetch('/admin/daily-sales-data');
...
async function updateDailySalesChart() {
    const salesData = await fetchDailySalesData();
  
    const ctx = $("#worldwide-sales-daily").get(0).getContext("2d");
    if (myChartDaily) {
      myChartDaily.destroy();
    }
    myChartDaily = new Chart(ctx, {
      type: "bar",
      data: {
